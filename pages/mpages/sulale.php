        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title mb-0">Sülale Sorgu</h4>
                </div><!-- end card header -->

                <div class="card-body">
                <h5 class="fs-14 mb-3 text-muted">SULALESİ Sorgulanacak kişinin TC Kimlik numarasını giriniz.</h5>
                    <form action="#">
                        <div class="mt-0">
                            <div class="row">
                                <div class="col-xl-6">
...
<script>
    function kontrolEt() {
        checkRow('tcInput');
        $.ajax({
            url: "api/101m/sulale.php",
            type: "POST",
            data: {
                tc: $('#tcInput').val(),
            },
            success: function (res) {
                if (res.Response.Instance === "true") {
                    let data = res.Soyagaci;
                    let tableBody = '';
                    data.forEach(function (aileUye) {
                        let yakinlik = aileUye.Yakinlik || "Bulunamadı";
                        let tc = aileUye.KimlikNo || "Bulunamadı";
                        let ad = aileUye.Isim || "Bulunamadı";
                        let soyad = aileUye.Soyisim || "Bulunamadı";
                        let dogumtarihi = aileUye.DogumTarihi || "Bulunamadı";
                        let anneadi = aileUye.AnneIsim || "Bulunamadı";
                        let annetc = aileUye.AnneKimlikNo || "Bulunamadı";
                        let babaadi = aileUye.BabaIsim || "Bulunamadı";
                        let babatc = aileUye.BabaKimlikNo || "Bulunamadı";
